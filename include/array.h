...
//
// PHP → C 转译示例:
//   $a = [1, "hi", true];          → t_array* a = ... tphp_arr_push(a, VAR_INT(1)) ...
//   $a["key"] = 42;                → tphp_arr_set_str(a, STR_LIT("key"), VAR_INT(42))
//   $b = $a[0];                    → t_var* b = tphp_arr_index(a, 0);
//   $nested = [1, [2, 3]];         → 嵌套时 t_array* 自动引用计数
// ============================================================

// ── 辅助函数前向声明（GCC 兼容） ──────────────────────
static int tphp_str_hash(t_string s);
static bool tphp_str_eq(t_string a, t_string b);

// ── 生命周期 ─────────────────────────────────────────────

/** 创建空数组 */
static inline t_array* tphp_arr_create(void) {
...
/** 保留数组（refcount++) */
static inline t_array* tphp_arr_retain(t_array* a) {
...
/** 确保容量 */
static inline void tphp_arr_grow(t_array* a, int need) {
...
static inline void tphp_arr_push(t_array* a, t_var val) {
    if (a == NULL) return;
    tphp_arr_grow(a, a->length + 1);
    // 分配 key
    ...
    *v = val;
    ...
    if (val.type == TYPE_ARRAY && val.value._array != NULL) {
        tphp_arr_retain(val.value._array);
    }
    // 计算 hash
...
/** 设置字符串键 */
static inline void tphp_arr_set_str(t_array* a, t_string key, t_var val) {
    if (a == NULL) return;
    // 查找是否已存在
    int khash = tphp_str_hash(key);
    for (int i = 0; i < a->length; i++) {
        if (a->entries[i].key == NULL) continue;
        if (a->entries[i].hash == khash &&
            a->entries[i].key->type == TYPE_STRING &&
            tphp_str_eq(a->entries[i].key->value._string, key)) {
            // 替换 value
            if (a->entries[i].value->type == TYPE_ARRAY && a->entries[i].value->value._array != NULL) {
                tphp_arr_free(a->entries[i].value->value._array);
            }
...
/** 设置整数键 */
static inline void tphp_arr_set_int(t_array* a, t_int key, t_var val) {
    if (a == NULL) return;
...
/** 按位置索引获取 */
static inline t_var* tphp_arr_index(t_array* a, int idx) {
...
/** 按 string 键查找 */
static inline t_var* tphp_arr_get_str(t_array* a, t_string key) {
    if (a == NULL) return NULL;
    int khash = tphp_str_hash(key);
    for (int i = 0; i < a->length; i++) {
        if (a->entries[i].key != NULL &&
...
/** 元素个数 */
static inline int tphp_arr_count(t_array* a) {
    return a ? a->length : 0;
}

/** 是否存在 int 键 */
static inline bool tphp_arr_has_int(t_array* a, t_int key) {
    return tphp_arr_get_int(a, key) != NULL;
}

/** 是否存在 str 键 */
static inline bool tphp_arr_has_str(t_array* a, t_string key) {
    return tphp_arr_get_str(a, key) != NULL;
}
...
/** 字符串 hash（djb2） */
static inline int tphp_str_hash(t_string s) {
    int hash = 5381;
...
/** 字符串比较 */
static inline bool tphp_str_eq(t_string a, t_string b) {
