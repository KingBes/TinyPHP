...
static inline t_int pcntl_fork(void) {
    pid_t pid = fork();
    return (t_int)pid;
}

static inline t_int pcntl_waitpid(t_int pid, t_int *status, t_int options) {
    return (t_int)waitpid((pid_t)pid, status, (int)options);
}

static inline t_int pcntl_wait(t_int *status) {
...
static inline t_int pcntl_alarm(t_int sec) {
    return (t_int)alarm((unsigned int)(sec > 0 ? sec : 0));
}

static inline t_int pcntl_get_last_error(void) {
    return (t_int)errno;
}
